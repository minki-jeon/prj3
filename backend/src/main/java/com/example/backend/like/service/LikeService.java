...
public class LikeService {
    ...
    public void update(LikeForm likeForm, Authentication authentication) {
        ...
        var boardLike = boardLikeRepository.findByBoardIdAndMemberEmail(likeForm.getBoardId(), authentication.getName());
    ...
    public BoardLikeDto get(Integer boardId, Authentication authentication) {
        ...
        Boolean liked = false;
        if (authentication != null) {
            var row = boardLikeRepository.findByBoardIdAndMemberEmail(boardId, authentication.getName());
            liked = row.isPresent();
        }
